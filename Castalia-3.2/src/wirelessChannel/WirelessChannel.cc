...
void WirelessChannel::initialize(int stage)
{
	...
	/* initialize the pathloss over time file read */
	if (strlen(pathLossFile) != 0) {
		pl_file.open(pathLossFile);
		if (!pl_file.is_open())
			opp_error("\n[Wireless Channel]:\n Error reading from pathLossFile %s\n", pathLossFile);
		else {
			readPathLoss();
			scheduleAt(simTime() + pl_update_interval, new cMessage("Periodic wireless channel update message", WC_PL_UPDATE));
		}
	}
...
void WirelessChannel::handleMessage(cMessage * msg)
{
	...
	case WC_SIGNAL_START:{
			...
			for (it1 = pathLoss[cellTx].begin(); it1 != pathLoss[cellTx].end(); it1++) {
				...
				trace() << "Ali: crrentSignalReceived: " << currentSignalReceived << " from node: " << srcAddr;
...
void WirelessChannel::readIniFileParameters(void)
{
	...
	maxTxPower = 0.0;
	pl_update_interval = par("pathLossUpdateInterval");
...
void WirelessChannel::readPathLoss(void)
{
	string s;
	const char *ct;
	int source = 0;
	...
	// each line of file is all elements of pathloss at one point of time
	if (!getline(pl_file, s))
	{
		pl_file.clear();
		pl_file.seekg(0, ios::beg);
		getline(pl_file, s);
	}
	ct = s.c_str();
	cStringTokenizer t(ct, ",");
	while ((ct = t.nextToken())) {
		pathloss_db = stof(ct);
		trace() << "New pathloss: " << source << ", " << destination << ", " << pathloss_db;
		updatePathLossElement(source, destination++, pathloss_db);
		if (destination == numOfNodes)
		{
			source++;
			destination = 0;
		}
